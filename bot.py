...
class MedicalBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        # Copie et synchronisation rapide sur votre serveur spécifique
        self.tree.copy_global_to(guild=GUILD_ID)
        await self.tree.sync(guild=GUILD_ID)
        logger.info(f"Commandes Slash synchronisées instantanément sur le serveur {GUILD_ID.id}.")
...
# ---------- CONFIGURATION DU BOT ET SETUP HOOK ----------
class MedicalBot(commands.Bot):
    def __init__(self):
        super().__init__(command_prefix="!", intents=intents)
    ...
    async def setup_hook(self):
        ...
        # (Peut prendre jusqu'à 1 heure pour se propager partout sur Discord)
...
# ---------- ÉVÉNEMENTS & GESTION DES ERREURS ----------
@bot.event
async def on_ready():
    logger.info(f"Connecté en tant que {bot.user} (ID: {bot.user.id})")
