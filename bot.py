...
FACTURATION_TARIFS = {
    "bandage": {"label": "Bandage / Pansement", "prix": 50},
    "attelle": {"label": "Pose d'attelle", "prix": 80},
    "platre": {"label": "Pose de plâtre", "prix": 150},
    "suture": {"label": "Suture", "prix": 200},
    "perfusion": {"label": "Perfusion", "prix": 100},
    "antidouleur": {"label": "Injection antidouleur", "prix": 60},
    "oxygene": {"label": "Oxygénothérapie", "prix": 120},
    "reanimation": {"label": "Réanimation", "prix": 500},
    "chirurgie": {"label": "Intervention chirurgicale", "prix": 1000},
    "transport": {"label": "Transport en ambulance", "prix": 200},
}


class FacturationSelect(discord.ui.Select):
    def __init__(self):
        options = [
            discord.SelectOption(label=f"{v['label']} — {v['prix']} €", value=k)
            for k, v in FACTURATION_TARIFS.items()
        ]
        super().__init__(
            placeholder="Sélectionne les soins effectués...",
            min_values=1,
    ...
    async def callback(self, interaction: discord.Interaction):
        view: FacturationView = self.view
        total = sum(FACTURATION_TARIFS[key]["prix"] for key in self.values)
        detail = "\n".join(
            f"• {FACTURATION_TARIFS[key]['label']} — {FACTURATION_TARIFS[key]['prix']} €"
            for key in self.values
        )
        patient_name = view.patient.display_name if view.patient else "Non renseigné"

        embed = discord.Embed(title="🧾 Facturation des soins", color=discord.Color.green())
        embed.add_field(name="Patient", value=patient_name, inline=False)
        embed.add_field(name="Soins effectués", value=detail, inline=False)
        embed.add_field(name="Total", value=f"**{total} €**", inline=False)

        record_id = await save_dossier_intervention(
            patient_user_id=view.patient.id if view.patient else None,
            patient_name=patient_name,
...
class FacturationView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=180)
        self.patient = None
        self.add_item(FacturationSelect())
...
async def facturation(interaction: discord.Interaction):
    embed = discord.Embed(
        title="🧾 Facturation des soins",
        description="Sélectionne un ou plusieurs soins effectués dans le menu ci-dessous pour calculer le total.",
        color=discord.Color.green(),
    )
    await interaction.response.send_message(embed=embed, view=FacturationView())
