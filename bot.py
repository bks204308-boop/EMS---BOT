...
def _pdf_safe(text: str) -> str:
    """La police standard du PDF encode en Latin-1, qui ne contient pas le symbole €
    (et peut ne pas contenir d'autres caractères spéciaux). On les neutralise ici
    pour éviter un crash silencieux à l'export."""
    if text is None:
        return ""
...
class SafeView(discord.ui.View):
    """Vue de base qui répond proprement à l'utilisateur en cas d'exception,
    au lieu de laisser Discord afficher 'n'a pas répondu à temps'."""
...
class DossierInterventionModal(discord.ui.Modal, title="Dossier d'Intervention"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Rappel automatique du dossier personnel du patient (allergies / groupe sanguin)
        if self.patient:
...
FACTURATION_CATEGORIES = {
    ...
    "analyses": {
        "label": "Analyses et suivis médicaux",
        "items": {
            "analyse_urine": {"label": "Analyse d'urine", "prix": 50},
            "analyse_sang": {"label": "Analyse de sang", "prix": 100},
            "points_suture": {"label": "Pose de points de suture", "prix": 500},
            "remplacement_suture": {"label": "Remplacement fil de suture", "prix": 250},
    ...
    "materiel": {
        ...
        "items": {
            "compresses": {"label": "Compresses stériles", "prix": 50},
            "bandages": {"label": "Bandages", "prix": 50},
            "kit_premiers_secours": {"label": "Nécessaire de premiers secours", "prix": 150},
            "kit_suture": {"label": "Nécessaire points de suture (petites coupures)", "prix": 150},
            "kit_intubation": {"label": "Kit d'intubation", "prix": 200},
            "pansements": {"label": "Pansements", "prix": 25},
            "poche_froid": {"label": "Poche de froid", "prix": 50},
            "kit_platre_attelle": {"label": "Kit plâtre / attelle", "prix": 200},
        },
    },
    "meds_standards": {
        "label": "Médicaments sans addiction (par jour)",
        "items": {
    ...
    "meds_addictifs": {
        "label": "Médicaments à risque d'addiction",
        "items": {
            "tramadol": {"label": "Tramadol (par jour, accord requis)", "prix": 150},
            "morphine": {"label": "Morphine (accord requis, prix à définir)", "prix": 0},
            "loprazolam": {"label": "Loprazolam (accord requis, prix à définir)", "prix": 0},
...
class FacturationSession:
    """Garde le total et le détail en cours de constitution pour une facturation."""
...
def build_facturation_embed(session: FacturationSession, note: Optional[str] = None) -> discord.Embed:
    embed = discord.Embed(title="🧾 Facturation des soins", color=discord.Color.green())
    if session.details:
        embed.add_field(
...
async def facturation(interaction: discord.Interaction):
...
class CaseSelect(discord.ui.Select):
    def __init__(self, zone_key: str):
        ...
        # Cas urgents affichés en premier dans la liste
...
async def on_ready():
    ...
    # Purge les commandes enregistrées globalement (doublons avec les commandes par serveur).
    # Sans danger de le laisser en permanence : si aucune commande globale n'existe déjà,
